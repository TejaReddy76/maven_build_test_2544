...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "jenk1-ear.ear");

        // Current directory is the root of the "jenk1-web" project. Go up one level
        // and enter the "jenk1-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../jenk1-ejb/target/jenk1-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "jenk1-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/jenk1-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "jenk1-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
